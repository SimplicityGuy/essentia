...
class EssentiaBuildExtension(build_ext):
    def run(self):
        ...
        # Route the compilation through sccache (see `waf configure --help`).
        # Set by the CI wheel builds; the sccache binary must be on PATH.
        sccache_flags = []
        if os.getenv('ESSENTIA_WITH_SCCACHE') == '1':
            sccache_flags = ['--with-sccache']
        ...
        if var_only_python in os.environ and os.environ[var_only_python]=='1':
            ...
            subprocess.run([PYTHON,  'waf', 'configure', '--only-python', '--static-dependencies',
                      '--prefix=tmp'] + macos_arm64_flags + sccache_flags, check=True)
        else:
            subprocess.run([PYTHON, 'waf', 'configure', '--build-static', '--static-dependencies',
                      '--with-python', '--prefix=tmp'] + macos_arm64_flags + sccache_flags, check=True)
